...
fn main() {
    ...
    let rc_exe = env::var_os("DISKTREE_RC").map(PathBuf::from).unwrap_or_else(|| {
        ...
        let mut versions: Vec<_> = fs::read_dir(&sdk_bin)
            ...
            .collect();
        versions.sort();
        versions.pop().expect("rc.exe is missing from the Windows SDK")
    });
    ...
    assert!(status.success(), "rc.exe failed to compile the Windows icon");
